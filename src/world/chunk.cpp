      ...
      int height = std::floor(
          TERRAIN_CONSTANT *
              noise.GetNoise(static_cast<float>(x) + chunk_x * CHUNK_SIZE,
                             static_cast<float>(z) + chunk_z * CHUNK_SIZE) +
          TERRAIN_CONSTANT);

      int local_height =
          std::min(CHUNK_SIZE, std::max(0, height - chunk_y * CHUNK_SIZE));

      for (int y = 0; y < local_height; ++y) {
        blocks[convert_to_block_idx(x, y, z)] = Block::DIRT;
      }
    }
  }
...
void Chunk::generate_mesh()
{
  ...
  for (int i = 0; i < CHUNK_SIZE_CUBED; ++i) {
    ...
    if (z - 1 < 0 || blocks[convert_to_block_idx(x, y, z - 1)] == Block::AIR) {
      for (int j = 0; j < 6; ++j) {
        unsigned int packed = ((face_nz[j][0] + x) << 24u) |
                              ((face_nz[j][1] + y) << 16u) |
    ...
    // Negative X
    if (x - 1 < 0 || blocks[convert_to_block_idx(x - 1, y, z)] == Block::AIR) {
      for (int j = 0; j < 6; ++j) {
        unsigned int packed = ((face_nx[j][0] + x) << 24u) |
                              ((face_nx[j][1] + y) << 16u) |
    ...
    // Negative Y
    if (y - 1 < 0 || blocks[convert_to_block_idx(x, y - 1, z)] == Block::AIR) {
      for (int j = 0; j < 6; ++j) {
        unsigned int packed = ((face_ny[j][0] + x) << 24u) |
                              ((face_ny[j][1] + y) << 16u) |
